...
const PaywallComponent = ({ ctx, userToken, clientToken, companyToken }) => {
    ...
    const finalClientData = {
        ...
        document: ctx.newClient?.document ? ctx.newClient?.document : userToken?.document,
        email: ctx.newClient?.email ? ctx.newClient?.email : userToken?.User?.email,
        company: ctx.newClient?.companyID ? ctx.newClient?.companyID : userToken?.companiesId ? userToken?.companiesId : companyToken?.id,
        packageId: ctx.clientChoice?.packageId ? ctx.clientChoice?.packageId : clientToken?.packageId,
        packageName: ctx.clientChoice?.packageName ? ctx.clientChoice?.packageName : clientToken?.packageName,
        packageType: ctx.clientChoice?.packageType ? ctx.clientChoice?.packageType : clientToken?.packageType,
        services: ctx.clientChoice?.services > 0 ? ctx.clientChoice?.services : clientToken.services,
        totalValue: ctx.clientChoice?.totalValue ? ctx.clientChoice?.totalValue : clientToken?.totalValue,
        maxInstallments: ctx.clientChoice?.maxInstallments ? ctx.clientChoice?.maxInstallments : clientToken?.maxInstallments,
        ccName: name,
        ccExpiry: expiry,
        ccNumber: cardNumber,
        ccCode: cvc
                    ...
                    >
                        <div style={{ display: "flex", alignItems: "center" }}>
                            <Typography variant='subtitle1' sx={{ fontWeight: 500 }}>
                                Olá, <span style={{ color: "#751fc5", fontSize: "22px" }}>{finalClientData.name}</span>
                            </Typography>
                        </div>
                    </AccordionSummary>
                    <Divider sx={{ m: '0 !important' }} />
                    <AccordionDetails sx={{ pt: 6, pb: 6 }}>
                        <Box sx={{ width: '100%', display: "flex", justifyContent: "space-around", backgroundColor: " #eeeeee", padding: "5px", borderRadius: "12px" }}>
                            <Box sx={{ width: '95%', wordWrap: "inherit" }}>
                                <span class="profile-paywall-section"><h1>Nome: </h1><h2>{finalClientData.name}</h2></span>
                                <span class="profile-paywall-section"><h1>Documento: </h1><h2>{finalClientData.document}</h2></span>
                                <span class="profile-paywall-section"><h1>E-mail: </h1><h2>{finalClientData.email}</h2></span>
                            </Box>
                        ...
                        <Box sx={{ width: '100%', marginTop: 10 }}>
                            {finalClientData.name && finalClientData.document && finalClientData.email ?
                                ""
                                :
                        ...
                        >
                            ...
                            <Box sx={{ width: '100%' }}>
                                ...
                                {finalClientData?.services?.map((e, y) => {
                                    ...
                                    return (
                                        <Typography sx={{ backgroundColor: "#dadada", fontSize: "12px", margin: "10px 0", width: "100%", fontWeight: "bold", borderRadius: "10px", padding: "5px 0 5px 5px" }}>{e.Service?.description}</Typography>
                                    )
                                }
                                )}
                                <Typography variant='body2' sx={{ fontSize: "11px" }}>Para ver o contrato,
                                    <a href="/mark/documents/contrato-mark-digital-12-meses.pdf" target="_blank"
                                ...
                                <Grid container spacing={6}>
                                    <Grid item xs={12}>
                                        ...
                                        >
                                            ...
                                            <FormControlLabel value='cash' control={<Radio />} label='Pix' />
                                        </RadioGroup>
                                    </Grid>
                                    {paymentMethod === 'card' ? (
                                        <Grid item xs={12}>
